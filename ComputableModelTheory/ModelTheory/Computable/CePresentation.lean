...
**Nonemptiness boundary (deliberate):** a total enumeration has nonempty range, so
c.e. presentations represent only *nonempty* structures — matching the all-ℕ layer,
where every presentation has carrier ℕ. Empty members of relational ages (the
substructure generated by the empty tuple) are a concern of the age-representation
layer, which can add a local empty marker or an `Option`-valued enumeration variant if
a reconciliation target demands it; the Level-2 initial-segment types inherit this
convention (`DomainShape.finite n` with `n ≠ 0` never asserted but `finite 0`
unreachable from an upgrade). -/
