...
theorem test_collapsing_static : (threeWidthFamily O).StaticAdmissible collapsingCandidate :=
...
theorem test_collapsing_carrierValid (d : ℕ → ℕ) :
  ...
  collapsingCandidate_carrierValid O d
...
theorem test_collapsing_not_embedding (d : ℕ → ℕ) :
    ¬ (threeWidthFamily O).PartialIsEmbedding (collapsingCandidate.chainMap d) :=
  collapsingCandidate_not_partialIsEmbedding O d
