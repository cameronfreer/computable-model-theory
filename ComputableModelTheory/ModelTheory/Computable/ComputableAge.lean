...
/-- The pair `[0, 1]` generates the successor structure: relabel the one-generator
witness terms through the initial-segment inclusion. -/
theorem succ_pair_generates : Tuple.Generates succLang ([0, 1] : Tuple ℕ) := by
  rw [Tuple.generates_iff]
  intro x
  obtain ⟨t, ht⟩ := (Tuple.generates_iff _).1 succ_tuple_generates x
  refine ⟨t.relabel (Fin.castLE (by decide)), ?_⟩
  rw [Term.realize_relabel]
  convert ht using 2
  funext i
  rw [show i = ⟨0, by decide⟩ from Fin.ext (Nat.lt_one_iff.1 (i.isLt : (i : ℕ) < 1))]
  rfl

/-- The triple `[0, 1, 2]` generates the successor structure, by the same relabelling. -/
theorem succ_triple_generates : Tuple.Generates succLang ([0, 1, 2] : Tuple ℕ) := by
  rw [Tuple.generates_iff]
  intro x
  obtain ⟨t, ht⟩ := (Tuple.generates_iff _).1 succ_tuple_generates x
  refine ⟨t.relabel (Fin.castLE (by decide)), ?_⟩
...
/-- The two-generator successor age: the same successor presentations, generated by
`[0, 1]`. Unlike the one-generator `succAge` — where every well-formed potential
embedding code is actual — well-formed nonembedding codes exist against it, making it
the standard fixture for exercising conditional soundness and totalization clauses on
degenerate inputs. -/
def succAge2 (O : Set (ℕ →. ℕ)) : ComputableAgeIn O succLang :=
  { succAge O with
    gens := fun _ ↦ [0, 1]
    gens_computableIn := ComputableIn.const [0, 1]
    generates := fun _ ↦ succ_pair_generates }

/-- The successor structure at every index, generated by `[0]` at index `0`, by `[0, 1]` at index
`1`, and by `[0, 1, 2]` elsewhere. Distinct generator widths are needed for the `(n+1)`-clause to
be satisfiable at all, and a two-generator source with a three-generator target is what lets a
width-correct candidate collapse two generators. -/
def succAgeMixed (O : Set (ℕ →. ℕ)) : ComputableAgeIn O succLang :=
  { succAge O with
    gens := fun i ↦ if i = 0 then [0] else if i = 1 then [0, 1] else [0, 1, 2]
    gens_computableIn :=
      ComputableIn.ite (c := fun i : ℕ ↦ i = 0)
        ...
        (ComputableIn.ite (c := fun i : ℕ ↦ i = 1)
          (((Primrec.eq (α := ℕ)).decide.to_comp.computableIn₂ (O := O)).comp
            ComputableIn.id (ComputableIn.const 1))
